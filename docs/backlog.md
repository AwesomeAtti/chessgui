...
| B-048 | chore | P0 | approved | Spike: board render + move interaction while a UCI engine floods stdout | Timeboxed, throwaway. Spec: `docs/feature-specs/b048-webview-engine-spike.md`. **macOS only** — no Linux hardware, so this validates the throttling architecture, not WebKitGTK. Linux half is B-066 |
...
| B-067 | tech-debt | P1 | proposed | Throttle high-frequency backend→frontend streams in Rust, not the frontend | Generalises the B-048 finding: parse UCI in Rust, hold latest state, emit on a fixed timer (~10 Hz). Per-line IPC events would melt any webview, Chromium included. Record in `docs/architecture.md` when it exists |
